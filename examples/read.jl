...
function main(;
    ...
    classifiers = FD.learn(
        pos_training_path, neg_training_path, features, votes, num_classifiers
    )
    ...
    correct_faces = sum(FD.ensemble_vote_all(
        pos_testing_path, classifiers; scale = scale, scale_to = scale_to
    ))
    correct_non_faces =
        num_non_faces - sum(FD.ensemble_vote_all(
            neg_testing_path, classifiers; scale = scale, scale_to = scale_to
        ),)
