...
function main(
    ...
    faces_scores[:] .= (get_faceness(classifiers, load_image(face, scale=scale, scale_to=scale_to)) for face in pos_testing_images)
	non_faces_scores[:] .= (get_faceness(classifiers, load_image(non_face, scale=scale, scale_to=scale_to)) for non_face in neg_testing_images)
    ...
    write(data_file, DataFrame(hcat(face_names, df_faces, non_face_names, df_non_faces), :auto), writeheader=false)
