...
function main(
    ...
    pareidolia_testing_images =
        load_image.(pareidolia_testing_image_paths, scale = scale, scale_to = scale_to)
    flower_testing_images =
        load_image.(flower_testing_image_paths, scale = scale, scale_to = scale_to)
    object_testing_images =
        load_image.(object_testing_image_paths, scale = scale, scale_to = scale_to)
    ...
    for (image_type, image_names, image_scores, faceness_scores) in (
        ...
        (FLOWER, flower_names, flower_scores, flower_faceness_scores),
        (OBJECT, object_names, object_scores, object_faceness_scores),
    )
...
data_file = joinpath(
    dirname(@__DIR__),
    "data",
    "classifiers_10_from_1500_pos_1500_neg_(128,128)_(128,128,1,1)",
)
