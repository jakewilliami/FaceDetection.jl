...
function main(
    ...
    face_scores = Float64[
        get_faceness(classifiers, load_image(img, scale = scale, scale_to = scale_to))
        for img in face_testing_images
    ]
    pareidolia_scores = Float64[
        get_faceness(classifiers, load_image(img, scale = scale, scale_to = scale_to))
        for img in pareidolia_testing_images
    ]
    flower_scores = Float64[
        get_faceness(classifiers, load_image(img, scale = scale, scale_to = scale_to))
        for img in flower_testing_images
    ]
    object_scores = Float64[
        get_faceness(classifiers, load_image(img, scale = scale, scale_to = scale_to))
        for img in object_testing_images
    ]
